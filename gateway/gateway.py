...
# de luu tru va truy xuat cac tep vuot qua gioi han kich thuoc tai lieu BSON la 16MB
fs_videos = gridfs.GridFS(mongo_video.db)
fs_mp3s = gridfs.GridFS(mongo_mp3.db)

# Ket noi RabbitMQ (synchronous)
# Thong so dua tren https://pika.readthedocs.io/en/stable/examples/heartbeat_and_blocked_timeouts.html
...
def login():
    # giao tiep dong bo voi dich vu xac thuc
...
def upload():
    ...
    for _, file_to_upload in request.files.items():
        # tai file len  MongoDB +
        # giao tiep khong dong bo voi converter service
